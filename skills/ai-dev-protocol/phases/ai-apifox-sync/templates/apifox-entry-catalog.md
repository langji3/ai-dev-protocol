...
## 八、Apifox CLI 同步计划（仅 CLI 模式保留）

> 本节是外部写入前的 dry plan，不代表已授权或已执行。不得包含 API token。
...
- 用户指定的已有模块：
- Apifox Branch：
- 当前 CLI 是否能明确指定该模块：是 / 否 / 待运行时核验
- CLI / 登录状态：可用 / 未安装 / 未登录 / 待检查
- 若无法可靠指定模块：停止写入，仅交付本计划

### 双目录映射

| 逻辑业务目录 | 推导依据 | Endpoint 目录 | Schema 目录 | 状态 |
| --- | --- | --- | --- | --- |
|  | 现有目录 / Controller / 路由分组 / 包名 |  |  | 复用 / 新建 / 待确认 |

### 幂等操作计划

| 顺序 | 资源类型 | 稳定匹配键 | 操作 | 目标目录 | 依赖 | 冲突 / 待确认 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | schema | 后端真实类型名 / 已知 ID | create / update / skip |  |  |  |
| 2 | endpoint | `Method + normalized Path` / 已知 ID | create / update / skip |  | schema ID |  |

### 写入边界

- 写入前必须获取当前 `cli-schema` 并逐个 validate payload。
- 默认不删除、不清理、不覆盖整模块、不 blanket import、不合并 Apifox 分支。
- 实际执行前必须由用户确认 Project、已有模块、Apifox Branch 和本表中的操作计划。
