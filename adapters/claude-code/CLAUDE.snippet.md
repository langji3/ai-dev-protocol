...
- Spec：实现前必须写中文 spec，并等待用户确认；个人分支模式下 spec 必须写入并提交到 `docs/specs/*.md`。
- 实现：个人分支模式下先创建未追踪的本地 plan，再拆分 plan/goals 并持续更新状态；极小改动可使用轻量 plan；只修改当前需求相关内容，不混入无关重构、格式化、依赖升级或 workflow 产物。
...
自然语言的模块设计讨论如果可能进入代码实现，也必须先作为需求进入处理。用户确认“个人分支”或“需求分支”只表示分支模式已确认，不表示允许实现；个人分支模式下下一步必须提交 `docs/specs/*.md` 并等待确认。
